...
#
#  KEY FIX (Final Version):
#    When churn labels are SYNTHETIC (no real churn column found), ML
#    accuracy / AUC-ROC are suppressed from all KPI cards and replaced
#    with an explicit "⚠ Synthetic Labels" warning.  This prevents
#    displaying meaningless self-referential metrics as real model quality.
#
#  Author   : Abhishek
...
try:
    ...
    # Guard: TotalCharges < MonthlyCharges is logically impossible
    ...
    _synthetic_churn = True      # default; flipped below if real column found
    ...
    if churn_col:
        ...
        risk_score = (
            (1 - df["tenure"].rank(pct=True, method="average")) * 0.40 +
            df["MonthlyCharges"].rank(pct=True, method="average") * 0.30 +
            df["SpendVariance"].rank(pct=True, method="average") * 0.20 +
            (1 - df["LoyaltyScore"].rank(pct=True, method="average")) * 0.10
        )
    ...
    accuracy   = None
    auc_score  = None
    model_name = "Heuristic (synthetic labels — no real churn column)" \
    ...
    if len(df) >= 20 and y.nunique() > 1:
        for Clf, name, kw in [
            ...
            try:
                ...
                # ── FINAL FIX: only record metrics when labels are REAL ────────
                # If churn is synthetic, accuracy/AUC measure how well the model
                # learned our own synthetic formula — not true churn prediction.
                # We still use the model's probabilities (better than heuristic)
                # but we do NOT surface misleading metric numbers.
                if not _synthetic_churn:
                    accuracy  = accuracy_score(ye, clf.predict(Xe))
                    auc_score = roc_auc_score(ye, prob_e) \
                                if ye.nunique() > 1 else None

                df["Churn_Prob"] = clf.predict_proba(X)[:, 1]
                model_name = name if not _synthetic_churn \
                             else f"{name} (Synthetic Labels — metrics suppressed)"
                break
            except Exception:
                continue
    ...
    # ── CLV — business-logic formula ──────────────────────────────────────────
    # CLV = TotalCharges × (1 – Churn_Prob)
    # A customer's expected future value is their historical spend discounted
    # by the probability they will leave.
    try:
    ...
    retention_rate  = round(min(100.0, max(0.0, 100 - churn_rate)), 2)
    ...
    revenue_at_risk = round(safe_float(
        df.loc[df["Churn_Prob"] >= 0.60, "TotalCharges"].sum()), 0)
    total_clv       = round(safe_float(df["Predicted_CLV"].sum()), 0)

    # Display strings — suppressed when synthetic
    if _synthetic_churn:
    ...
    def build_report(out_path):
        with pd.ExcelWriter(out_path, engine="xlsxwriter") as writer:
            ...
            F = {
                ...
                # Warning label for suppressed metrics
                ...
                # Warning value — smaller font, amber text, for suppressed metrics
            ...
            # Row heights: title/subtitle/KPI rows
            for r, h in {0: 5, 1: 46, 2: 20, 3: 8,
                         4: 18, 5: 48, 6: 8,
                         7: 18, 8: 48, 9: 12}.items():
                dash.set_row(r, h)

            title_last_col = N_CARDS * 2
            ...
            # Shown only when churn is synthetic so readers cannot miss the caveat.
            warn_row_start = 10
            if _synthetic_churn:
                dash.set_row(warn_row_start, 36)
                dash.merge_range(
                    warn_row_start, 1, warn_row_start, title_last_col,
                    "⚠  SYNTHETIC LABELS NOTICE:  No real churn column was found in "
                    "this dataset.  Churn labels were generated using a percentile "
                    "risk-score formula.  The ML model trained on these labels cannot "
                    "produce valid accuracy or AUC-ROC scores — those metrics are "
                    "suppressed.  Churn probabilities and segmentation are still "
                    "useful for relative risk ranking.",
                    F["warn_banner"],
                )
                chart_anchor_row = warn_row_start + 2   # charts start below warning
            else:
                chart_anchor_row = warn_row_start        # no warning → charts start here
            ...
            # ── FINAL FIX: accuracy/AUC cards use warning style when synthetic ─
            if _synthetic_churn:
            ...
            # ── Dashboard charts (positions anchored to chart_anchor_row) ───────
            r_charts1 = chart_anchor_row + 1
            r_charts2 = r_charts1 + 17
            r_charts3 = r_charts2 + 17

            def row_to_excel(r):
                """Convert 0-based row index to Excel cell reference for insert_chart."""
                return r

            add_chart("column",   sn_seg,  seg_n,
                      "Customer Segments by Count",
                      dash, f"B{r_charts1 + 1}",
                      fill_colors=CHART8, w=540, h=310)
            add_chart("doughnut", sn_risk, risk_n,
                      "Risk Tier Breakdown",
                      dash, f"J{r_charts1 + 1}",
            ...
            # Note on metrics validity — explicit in audit trail
            metrics_note = (
                "SUPPRESSED — churn labels are synthetic; metrics would "
                "measure formula self-consistency, not true predictive power."
            ...
            pipeline_rows = [
                ...
                ("row", "Churn Labels Valid",     "NO — synthetic formula" if _synthetic_churn
                ...
                ("row", "Metrics Suppressed",     "YES — synthetic labels" if _synthetic_churn
                                                  else "NO — real labels used"),
                ...
                ("row", "Revenue at Risk",
                 f"Rs.{revenue_at_risk:,.0f}  (TotalCharges of High-Risk customers)"),
    ...
    synthetic_note = (
        "\n** NOTE: Churn labels were SYNTHETIC — ML accuracy/AUC suppressed. **\n"
        "   Risk scores and segmentation are still valid for relative ranking.\n"
    ...
    info(
        ...
        f"  High-Risk Customers  :  {high_risk_n:,}  ({high_risk_pct}%)\n"
        f"  Revenue at Risk      :  Rs.{revenue_at_risk:,.0f}\n"
        ...
        f"  Data – Segments/Risk   7 chart-source sheets",
    )
