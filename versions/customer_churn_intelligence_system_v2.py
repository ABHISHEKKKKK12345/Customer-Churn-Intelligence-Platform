...
from sklearn.linear_model import LinearRegression  # retained for potential future use
...
try:
    ...
    # Guard: if TotalCharges < MonthlyCharges it is logically impossible
    # (a customer cannot have paid less in total than one month's charge).
    # This can happen when pick() falls back to the wrong numeric column.
    # In that case recompute from tenure × MonthlyCharges.
    ...
    # ── Feature engineering ────────────────────────────────────────────────────
    # NOTE: computed BEFORE churn mapping so the synthetic risk_score fallback
    # can reference SpendVariance and LoyaltyScore correctly.
    #
    # AvgMonthlySpend: total spend spread over active months (+1 avoids /0)
    df["AvgMonthlySpend"] = (
        df["TotalCharges"] / (df["tenure"] + 1)
    ).clip(lower=0).fillna(0)

    # ValueScore: log1p(TotalCharges) — stable, avoids tenure-driven inflation.
    # log1p(0) = 0 so zero-spend customers score 0 cleanly.
    df["ValueScore"] = np.log1p(df["TotalCharges"]).clip(lower=0).fillna(0)

    # LoyaltyScore: tenure x spending commitment — rewards long-term high spenders.
    # Old formula (tenure / MonthlyCharges) unfairly penalised high-paying customers.
    df["LoyaltyScore"] = (
        df["tenure"] * (df["TotalCharges"] / (df["MonthlyCharges"] + 1))
    ).clip(lower=0).fillna(0)

    # SpendVariance: relative deviation (normalised) — comparable across datasets.
    # Old absolute difference was not scale-invariant.
    df["SpendVariance"] = (
    ...
    if churn_col:
        ...
        # Multi-factor risk score — uses pre-computed engineered features.
        # method="average" ensures consistent tie-handling across all datasets.
        # Weights reflect business importance:
        #   tenure (40%)        — short-tenure customers churn most
        #   MonthlyCharges (30%)— high charges increase price-sensitivity
        #   SpendVariance (20%) — erratic spend signals dissatisfaction
        #   LoyaltyScore (10%)  — low loyalty amplifies churn risk
        risk_score = (
        ...
        # Percentile-based threshold: always labels the top 35% as churners.
        # More robust than a fixed 0.5 cut-off which is distribution-dependent.
    ...
    accuracy   = None
    auc_score  = None
    # If churn labels are synthetic, flag that upfront so the report is honest:
    # ML trained on synthetic labels learns our own assumptions, not true churn.
    _synthetic_churn = "Detected:" not in churn_source
    model_name = "Heuristic (Synthetic Labels — No True Churn Column)" if _synthetic_churn else "Heuristic"
    ...
    if len(df) >= 20 and y.nunique() > 1:
        for Clf, name, kw in [
            ...
            try:
                ...
                model_name = name if not _synthetic_churn else f"{name} (Synthetic Labels)"
                break
            except Exception:
    ...
    # Formula: CLV = TotalCharges * (1 - Churn_Prob)
    # Rationale: a customer's future value is their historical spend discounted
    # by the probability they will leave.  This avoids the circular-logic trap
    # of regressing TotalCharges on the same features used to compute it.
    try:
    ...
    retention_rate = round(min(100.0, max(0.0, 100 - churn_rate)), 2)  # clamp [0,100]
    ...
    # Sanity: high + med + low == total_cust (boundaries: ≥0.60=High, 0.30–0.59=Med, <0.30=Low)
    # Churn_Prob is clipped [0,1] and rounded to 4dp so no values fall outside these ranges.
    # Revenue at Risk = sum of TotalCharges for high-risk customers.
    # We use TotalCharges (not CLV) because this represents the actual historical
    # revenue that would be lost if these customers churn — not their discounted value.
    ...
    def build_report(out_path):
        with pd.ExcelWriter(out_path, engine="xlsxwriter") as writer:
            ...
            pipeline_rows = [
                ...
                ("row", "Revenue at Risk",         f"₹{revenue_at_risk:,.0f}  (TotalCharges of High-Risk customers)"),
