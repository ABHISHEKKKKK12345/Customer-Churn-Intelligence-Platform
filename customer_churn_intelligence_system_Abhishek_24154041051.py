...
def load_file(path):
    ...
    For Excel extensions, falls back to CSV parsing if all Excel engines fail
    (handles files mis-named as .xlsx that are actually CSV/TSV).
    ...
    if ext in (".xlsx", ".xls", ".xlsm", ".xlsb", ".ods"):
        ...
        # Some files carry an .xlsx/.xls extension but are actually plain-text
        # (CSV / TSV).  pandas raises "File is not a zip file" in that case.
        # We silently retry with every encoding × separator combination before
        ...
        raise RuntimeError(
            ...
            "Possible causes:\n"
            "  • The file has an .xlsx/.xls extension but is NOT a real Excel file\n"
