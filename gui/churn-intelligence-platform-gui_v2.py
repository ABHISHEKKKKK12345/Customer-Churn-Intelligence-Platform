...
#
#  AUDIT v2.0 — Production-grade corrected version.
#  All bugs fixed; full feature coverage preserved.
# ==============================================================================
...
class SplashScreen:
    def __init__(self):
        ...
        tk.Label(logo_frame, text="\U0001F4CA", font=("Segoe UI Emoji", 42),
...
class ChurnApp:
    def __init__(self):
        ...
        # FIX-1: Initialize feature_importance_df at instance level to avoid
        # AttributeError when ML is skipped (rows < MIN_ROWS_FOR_ML or 1 class).
    ...
    def _build_ui(self):
        ...
        tk.Label(hdr, text="\U0001F4CA  Customer Churn Intelligence Platform",
        ...
        desc_text = (
            ...
            "  \u2714  Segments customers by Risk x Value matrix\n"
        ...
        self._section("Step 1  \u2014  Select Your Dataset", "\U0001F4C2", pad)
        ...
        self._section("Step 2  \u2014  Choose Save Location", "\U0001F4BE", pad)
        ...
        self._section("Step 3  \u2014  Run Analysis", "\U0001F680", pad)
        ...
        self._section("Results  \u2014  Key Performance Indicators", "\U0001F4C8", pad)
        ...
        # -- Log area ----------------------------------------------------------
        self._section("Activity Log", "\U0001F4CB", pad)
    ...
    def _run_analysis(self):
        ...
        # FIX-1 (continued): Reset feature_importance_df before each run so
        # a prior successful run's data is not passed to the report if ML fails.
        self.feature_importance_df = None
    ...
    def _run_pipeline(self):
        ...
        self._log("ok", f"Loaded {original_rows:,} rows x {original_cols_n} columns")
        ...
        if churn_col and churn_col in df.columns:
            ...
            # FIX-2: Guard against division by zero when TotalCharges is all
            # zeros (e.g. very small/missing-data datasets).
            _safe_tc = df["TotalCharges"].replace(0, np.nan).fillna(1)
            _safe_ten = df["tenure"] + 1
            _sv = ((df["MonthlyCharges"] - (_safe_tc / _safe_ten))
                   / (_safe_tc / _safe_ten + 1)).abs().clip(lower=0).fillna(0)
            risk_score = (
        ...
        # FIX-3: Handle single-class scenario (Scenario E).
        # If dataset has only one churn class, force a minimal representation
        # so downstream ML and metrics don't silently break.
        if df["Churn"].nunique() < 2:
            self._log("warn", "Only one churn class detected — adding minimal counter-class rows for stability.")
            minority_val = 1 - int(df["Churn"].mode().iloc[0])
            n_inject = max(2, int(len(df) * 0.05))
            minority_idx = df.sample(n=min(n_inject, len(df)), random_state=42).index
            df.loc[minority_idx, "Churn"] = minority_val
            _synthetic_churn = True
            churn_source = churn_source + " [single-class correction applied]"
        ...
        # FIX-1 (continued): feature_importance_df is already initialised on
        # the instance (self.feature_importance_df = None in __init__ and reset
        # before each run). Use a local variable here as well and assign
        # self.feature_importance_df only when a model succeeds.
        local_feature_importance_df = None
        ...
        if len(df) >= MIN_ROWS_FOR_ML and y.nunique() > 1:
            for Clf, name, kw in [
                ...
                try:
                    ...
                    test_size   = 0.20
                    # FIX-4: stratify requires at least 2 samples per class in
                    # BOTH train AND test splits.  The prior check was not
                    # tight enough for small datasets (Scenario C).
                    n_test  = max(1, int(len(df) * test_size))
                    n_train = len(df) - n_test
                    do_stratify = (
                        min_class >= 2
                        and min_class <= n_train
                        and min_class <= n_test
                    )
                    ...
                    if hasattr(clf, "feature_importances_"):
                        local_feature_importance_df = pd.DataFrame({
                            "Feature": FEATURES,
                    ...
                    if not _synthetic_churn and accuracy is not None:
                        if auc_score is not None:
                            self._log("ok", f"Accuracy: {accuracy*100:.2f}%   AUC-ROC: {auc_score:.4f}")
                        else:
                            self._log("ok", f"Accuracy: {accuracy*100:.2f}%")
                    break
        ...
        # Propagate local result to instance attribute for report builder.
        self.feature_importance_df = local_feature_importance_df
    ...
    def _show_kpi_cards(self, total_cust, churn_rate, retention_rate,
        ...
        cards = [
            ("\U0001F465 Total Customers",        f"{total_cust:,}",               GUI["accent2"],  GUI["white"]),
            ("\U0001F4C9 Churn Rate",              f"{churn_rate}%",                GUI["red"],      GUI["white"]),
            ("\U0001F49A Retention Rate",          f"{retention_rate}%",            GUI["green"],    GUI["white"]),
            ("\U0001F534 High Risk  (>=60%)",      f"{high_risk_n:,}\n({high_risk_pct}%)", GUI["red"], GUI["white"]),
            ("\U0001F7E1 Medium Risk (30-60%)",    f"{med_risk_n:,}",               "#B7950B",       GUI["white"]),
            ("\U0001F7E2 Low Risk  (<30%)",        f"{low_risk_n:,}",               GUI["green"],    GUI["white"]),
            ("\u23F1 Avg Tenure",                  f"{avg_tenure} mo",              GUI["bg2"],      GUI["white"]),
            ("\U0001F4B0 Avg CLV",                 f"Rs.{avg_clv:,.0f}",            GUI["accent2"],  GUI["white"]),
            ("\u26A0 Risk Revenue/yr",             f"Rs.{revenue_at_risk:,.0f}",    GUI["red"],      GUI["white"]),
            ("\U0001F4CA Total CLV Portfolio",     f"Rs.{total_clv:,.0f}",          GUI["purple"],   GUI["white"]),
            ("\U0001F4B3 Avg Monthly",             f"Rs.{avg_monthly:,.0f}",        GUI["bg2"],      GUI["white"]),
            (
                "\U0001F3F7 Label Type" if _synthetic_churn else "\U0001F916 ML Accuracy",
                "Synthetic" if _synthetic_churn else acc_str,
        ...
        tk.Label(model_frame,
                 text=f"\U0001F916  Model: {model_name}   |   Accuracy: {acc_str}   |   AUC-ROC: {auc_str}",
                 font=("Segoe UI", 10),
...
# ==============================================================================
#  ANALYTICS UTILITIES  (no GUI dependency)
# ==============================================================================
def norm(s):
    ...
    if ext in (".xlsx",".xls",".xlsm",".xlsb",".ods"):
        ...
        for eng in engines:
            try:
                ...
                errors.append(str(ex))
        # ── FIX: CSV fallback for mis-named files ─────────────────────────────
        # Some files carry an .xlsx extension but are plain-text CSV/TSV inside.
        # pandas raises "File is not a zip file" in that case.
        # Retry silently with every encoding × separator before surfacing error.
        _ENCS = ("utf-8-sig", "utf-8", "latin1", "cp1252", "iso-8859-1", "utf-16")
        _SEPS = (None, ",", ";", "\t", "|", ":")
        for _enc in _ENCS:
            for _sep in _SEPS:
                try:
                    _df = _try_csv(path, _enc, _sep)
                    if _df is not None:
                        _df = _df.dropna(how="all").dropna(axis=1, how="all")
                        if len(_df) >= 1 and len(_df.columns) >= 2:
                            return _df
                except Exception as _exc:
                    errors.append(str(_exc))
        # ── end CSV fallback ──────────────────────────────────────────────────
        raise RuntimeError(
            "Cannot open this file as Excel/ODS or as a delimited text file.\n"
            "Possible causes:\n"
            "  • The file has an .xlsx extension but is NOT a real Excel file\n"
            "  • The file is password-protected or corrupted\n"
            "  • Try re-saving from Excel as .xlsx, or export a fresh .csv\n\n"
...
def build_excel_report(df, original_df, out_path, file_path,
    ...
    with pd.ExcelWriter(out_path, engine="xlsxwriter") as writer:
        ...
        # FIX-5: 'af2' was assigned but never used when not synthetic;
        # removed orphan variable.
        if _synthetic_churn:
            af,vf2="kpi_lbl_w","kpi_warn"; ad,auc_d="N/A - Synthetic","N/A - Synthetic"
        else:
            af,vf2="kpi_lbl_g","kpi_green"
        ...
        for i,(lbl,val,lf,vf) in enumerate([
            ...
            ("LABEL TYPE" if _synthetic_churn else "MODEL ACCURACY","Synthetic" if _synthetic_churn else ad,af,vf2),
            ("AUC-ROC SCORE",auc_d,af,vf2),]): kpi_card(dash,lbl,val,KLR2,KVR2,i,lf,vf)
        ...
        ws6.set_column(0,0,1.5); ws6.set_column(1,1,38); ws6.set_column(2,2,120)
        ...
        pipeline_rows=[
            ...
            ("row","CLV Methodology","Predicted CLV = MonthlyCharges x 12 x (1 - Churn Probability)."),
            ("row","Revenue at Risk",f"MonthlyCharges x 12 for Churn_Prob >= 60%.  Historical at risk: Rs.{tc_at_risk_hist:,.0f}."),
            ("row","Segmentation","2x2: Risk (>=0.60) x Value (>=median CLV)"),
            ("row","Synthetic Formula","Risk=tenure(down)x0.40+monthly(up)x0.30+variance(up)x0.20+total(down)x0.10.  Top 35%->Churned."),
            ("sec","OUTPUT KPIs"),
        ...
        # FIX-6: feature_importance_df may be None when ML was skipped (small
        # dataset, single class, or both models failed). The sheet is created
        # only when the DataFrame is available and non-empty.
        if feature_importance_df is not None and not feature_importance_df.empty:
            fi_ws = wb.add_worksheet(_trunc("Feature Importance"))
            fi_ws.hide_gridlines(2)
            fi_ws.set_zoom(100)
            fi_ws.set_column(0, 0, 30)
            fi_ws.set_column(1, 1, 20)
            # FIX-7: Ensure Importance column is numeric before writing;
            # sort descending so the most important feature is at the top.
            fi_df = feature_importance_df.copy()
            fi_df["Importance"] = pd.to_numeric(fi_df["Importance"], errors="coerce").fillna(0)
            fi_df = fi_df.sort_values("Importance", ascending=False).reset_index(drop=True)
            write_table(fi_ws, fi_df, 0, hdr_fmt="col_hdr_t")

        # -- Sheet 8: Executive Recommendations --------------------------------
        # FIX-8: The original code had five critical bugs:
        #   (a) Missing opening quote on "Maintain current retention strategy."
        #       → SyntaxError (the root crash of the whole file).
        #   (b) The three inner recommendations (Many High Risk, High Revenue,
        #       Strong Customer Value) and the DEFAULT block were all nested
        #       inside the `if high_risk_pct > 20:` block, meaning they could
        #       never be reached if high_risk_pct <= 20.
        #   (c) The DEFAULT `if not recommendations:` check was also inside the
        #       `if high_risk_pct > 20:` block — logically impossible to be
        #       True inside that block since high_risk_pct > 20 already added
        #       an item.
        #   (d) Duplicate default check (`if not recommendations` AND
        #       `if len(recommendations) == 0`) — redundant, one removed.
        #   (e) The rec_df creation, rec_ws add_worksheet, and write_table calls
        #       were also inside the `if high_risk_pct > 20:` block, so the
        #       sheet was NEVER generated when high_risk_pct <= 20.
        # All fixed below: each recommendation is independent; sheet is always
        # generated; default always fires when no other condition triggers.
        ...
        if churn_rate > 30:
            recommendations.append([
                "High Churn Rate",
                "Review customer experience and pricing strategy."
            ])
        ...
        if revenue_at_risk > 100000:
            recommendations.append([
                ...
                "Prioritize intervention for top-value customers."
            ])

        if avg_clv > 10000:
            recommendations.append([
                "Strong Customer Value",
                "Invest in loyalty and upselling programs."
            ])

        # DEFAULT: always present when no other condition triggered.
        if not recommendations:
            recommendations.append([
                "Healthy Portfolio",
                "Maintain current retention strategy."
            ])
        ...
        rec_ws = wb.add_worksheet(_trunc("Executive Recommendations"))
        rec_ws.hide_gridlines(2)
        rec_ws.set_zoom(100)
        rec_ws.set_column(0, 0, 30)
        rec_ws.set_column(1, 1, 80)
        write_table(rec_ws, rec_df, 0, hdr_fmt="col_hdr_p")
